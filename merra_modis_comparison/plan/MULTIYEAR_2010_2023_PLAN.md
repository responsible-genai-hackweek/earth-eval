...
Status on this branch: **specification only — not yet implemented.**

> **Provenance:** ported from the `clinton` branch (authored by Codex) on
> 2026-08-26 for an independent-implementation comparison. The body below is
> unchanged; only this note and the status line differ. On `clinton` this plan
> was marked *implemented and validated with an authenticated month-level smoke
> test*, and section 7 records that branch's validation history — on this branch
> every item there is a gate still to be met.
