...
- `slides/` — the nine-slide hackweek deck (LaTeX beamer, Anthropic orange
  theme). Its README carries the build command and conventions. Every number on
  a slide is taken from `results/FINDINGS.md`, so a rerun of the analysis
  obliges a pass over the deck.
