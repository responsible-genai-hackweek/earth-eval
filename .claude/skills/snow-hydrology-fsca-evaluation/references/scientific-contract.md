...
Pair records by calendar date. Do not substitute a daily mean MERRA value or a
...
3. Bin the center into one native MERRA-2 cell using the target cell edges.
...
pixels at MERRA boundaries. That approximation is appropriate because the fine
pixels are much smaller than the target cells. Do not use bilinear resampling
for the comparison reference.

## Error metrics

Let `M` be MERRA-2 fSCA, `R` be aggregated MODSCAG fSCA, and `w` be the valid
...
- Positive bias means MERRA-2 is too snowy; negative bias means it is too low.
...
- Daily MERRA and MODSCAG values must both be finite before an error contributes.
